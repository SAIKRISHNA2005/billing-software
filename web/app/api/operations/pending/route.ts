...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await callAppsScript('operations.pending', params, sessionToken);
    return NextResponse.json(result, { status: result.success ? 200 : 400 });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Failed to retrieve pending operations';
